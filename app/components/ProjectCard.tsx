...
export default function ProjectCard({ project, movedRef }: ProjectCardProps) {
  ...
  return (
    <article
      className="w-[85vw] sm:w-113 shrink-0 snap-center bg-surface p-6 rounded-3xl border border-text-muted flex flex-col"
      onClick={(e) => {
    ...
    >
      ...
      <div className="flex flex-col gap-2 flex-1 md:gap-3">
        ...
        <p className="text-sm text-text-soft max-w-90 flex-1">
          {project.blurb}
        </p>

        <div className="flex gap-3 flex-wrap mt-4 md:mt-6 md:mb-2">
          {project.tags.map((tag) => (
            <span
              key={tag}
              className="font-mono text-[10px] tracking-wider text-text-soft border border-border-strong px-3 py-1 rounded-lg"
            >
              {tag}
            </span>
          ))}
